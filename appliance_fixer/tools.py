"""Perception + model-number tools for Appliance Fixer.
...
def validate_model(model_number, brand=None):
    ...
    Applies normalize_model + canonicalize_symbols to both sides, and accepts a containment match
    (handles trailing revision letters). When `brand` is given and a MODEL_PATTERNS regex exists
    for it, a totally malformed input (failing the loose regex) returns None before membership.
    """
    ...
    # Loose per-brand regex sanity check (catches obviously malformed reads).
    if brand_u and brand_u in fridge.MODEL_PATTERNS:
        if not re.match(fridge.MODEL_PATTERNS[brand_u], norm):
            return None

    for b, models in fridge.SUPPORTED_MODELS.items():
